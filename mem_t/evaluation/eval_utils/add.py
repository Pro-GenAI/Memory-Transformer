...
class MemoryADD:
    def __init__(self, data_path=None, batch_size=2, is_graph=False):
        ...
        if not os.getenv("MEM0_BASE_URL"):
            self.mem0_client.project.update(custom_instructions=custom_instructions)
        ...
        if data_path:
            self.load_data()
        print("MemoryADD initialized")

    def load_data(self):
        with open(self.data_path, "r") as f:  # type: ignore
            self.data = json.load(f)
        print(f"Loaded data: {len(self.data)} conversations from {self.data_path}")
        return self.data
    ...
    def process_conversation(self, item, idx):
        ...
        for key in conversation.keys():
            ...
            # Avoid using threads to prevent rate limiting issues
